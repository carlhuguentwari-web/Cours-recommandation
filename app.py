...
st.write("Choisissez un cours dans la barre latérale, puis cliquez sur le bouton ci-dessous pour afficher les recommandations.")

choix = st.sidebar.selectbox("Choisissez un cours", cours['titre'])

valider = st.button("Afficher les recommandations")

if valider:
    id_choisi = cours[cours['titre'] == choix]['id'].values[0]
    description_choisie = cours.loc[cours['id'] == id_choisi, 'description'].iloc[0]
